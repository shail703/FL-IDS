...
def load_model_latest(model_dir="aggregator_models"):
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith(".h5")])
    if not model_files:
        raise FileNotFoundError("No model files found in aggregator_models/")
    return load_model(os.path.join(model_dir, model_files[-1]))
